...
class SupConLossWithQueue(nn.Module):
    def __init__(self, temperature=0.07, contrast_mode='all', base_temperature=0.07, queue_size=65536):
        super(SupConLossWithQueue, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
        self.queue_size = queue_size
        self.queue = None  # Initialize the queue later based on feature size

    def enqueue_dequeue(self, features):# TODO: test if only one part of feature added to the queue
        if self.queue is None:
            self.queue = torch.zeros(self.queue_size, *features.shape[1:], device=features.device)

        # Enqueue new features and dequeue oldest
        self.queue = torch.cat([features, self.queue[:-features.size(0)]], dim=0)
